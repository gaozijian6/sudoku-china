// 显性三数组法 2
func nakedTriple2(
    board: [[CellData]],
    candidateMap: CandidateMap,
    graph: Graph
) -> [String: Any]? {
    // 检查行
    if let rowResult = checkNakedTriple2(board: board, unitType: "row") {
        return rowResult
    }
    
    // 检查列
    if let colResult = checkNakedTriple2(board: board, unitType: "col") {
        return colResult
    }
    
    // 检查宫
    if let boxResult = checkNakedTriple2(board: board, unitType: "box") {
        return boxResult
    }
    
    return nil
}

func checkNakedTriple2(board: [[CellData]], unitType: String) -> [String: Any]? {
    for unit in 0..<9 {
        var cellsWithCandidates: [(pos: Position, candidates: [Int])] = []
        
        // 收集单元内的候选数和位置（只考虑有2个候选数的单元格）
        for i in 0..<9 {
            let row: Int
            let col: Int
            
            switch unitType {
            case "row":
                row = unit
                col = i
            case "col":
                row = i
                col = unit
            case "box":
                row = (unit / 3) * 3 + (i / 3)
                col = (unit % 3) * 3 + (i % 3)
            default:
                continue
            }
            
            let cell = board[row][col]
            if cell.value == nil && cell.draft.count == 2 {
                cellsWithCandidates.append((pos: Position(row: row, col: col), candidates: cell.draft))
            }
        }
        
        // 检查所有可能的三个格子组合
        for i in 0..<(cellsWithCandidates.count - 2) {
            for j in (i + 1)..<(cellsWithCandidates.count - 1) {
                for k in (j + 1)..<cellsWithCandidates.count {
                    let cellA = cellsWithCandidates[i]
                    let cellB = cellsWithCandidates[j]
                    let cellC = cellsWithCandidates[k]
                    
                    // 获取所有唯一的候选数
                    var uniqueCandidates = Set<Int>()
                    for num in cellA.candidates { uniqueCandidates.insert(num) }
                    for num in cellB.candidates { uniqueCandidates.insert(num) }
                    for num in cellC.candidates { uniqueCandidates.insert(num) }
                    
                    if uniqueCandidates.count == 3 {
                        let candidatesArray = Array(uniqueCandidates)
                        
                        var affectedPositions: [Position] = []
                        let prompt: [Position] = [cellA.pos, cellB.pos, cellC.pos]
                        
                        // 检查其他格子是否受影响
                        for m in 0..<9 {
                            let row: Int
                            let col: Int
                            
                            switch unitType {
                            case "row":
                                row = unit
                                col = m
                            case "col":
                                row = m
                                col = unit
                            case "box":
                                row = (unit / 3) * 3 + (m / 3)
                                col = (unit % 3) * 3 + (m % 3)
                            default:
                                continue
                            }
                            
                            let cell = board[row][col]
                            if cell.value == nil &&
                               !prompt.contains(where: { $0.row == row && $0.col == col }) &&
                               cell.draft.contains(where: { candidatesArray.contains($0) }) {
                                affectedPositions.append(Position(row: row, col: col))
                            }
                        }
                        
                        if !affectedPositions.isEmpty {
                            var method = ""
                            
                            switch unitType {
                            case "row":
                                method = SolutionMethods.NAKED_TRIPLE_ROW2
                            case "col":
                                method = SolutionMethods.NAKED_TRIPLE_COLUMN2
                            case "box":
                                method = SolutionMethods.NAKED_TRIPLE_BOX2
                            default:
                                method = "Unknown"
                            }
                            
                            return [
                                "position": affectedPositions.map { ["row": $0.row, "col": $0.col] },
                                "prompt": prompt.map { ["row": $0.row, "col": $0.col] },
                                "method": method,
                                "target": candidatesArray,
                                "isFill": false,
                                "row": cellA.pos.row,
                                "col": cellA.pos.col,
                                "box": (cellA.pos.row / 3) * 3 + (cellA.pos.col / 3)
                            ]
                        }
                    }
                }
            }
        }
    }
    
    return nil
}